...
def objective(trial):
    ...
    batch_size = trial.suggest_categorical('batch_size', [256, 512])  # Restrict batch size to 256 or 512
    ...
    data_fraction = trial.suggest_uniform('data_fraction', 0.05, 0.5)  # Tuning data_fraction

    # Generate the arguments for the train function
    args = parse_args()
    ...
    args.data_fraction = data_fraction  # Set tuned data_fraction
    ...
    # Ensure log and weight directories exist
...
def main():
    ...
    # Removed the --storage argument to enforce local storage
    ...
    # Define a local storage path for the Optuna study as a pickle file
    ...
    if os.path.exists(study_path):
        try:
            ...
            # If failed to load, create a new study
    ...
    torch.set_num_threads(2)  # Limit CPU thread usage
    ...
    # Retrieve and log trial results
